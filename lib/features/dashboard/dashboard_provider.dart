...
Future<DashboardState> dashboardData(DashboardDataRef ref) async {
  ...
  final txs = await (db.select(db.transactions)
    ..orderBy([(t) => OrderingTerm(expression: t.transactionDate, mode: OrderingMode.desc), (t) => OrderingTerm(expression: t.id, mode: OrderingMode.desc)])
    ..limit(5))
      .get();
  ...
  for (int i = 0; i < 6; i++) {
    ...
    final revenueQuery = db.selectOnly(db.transactions)
      ...
      ..where(db.transactions.type.equals('INC') & db.transactions.transactionDate.isBetweenValues(targetMonth, nextMonth.subtract(const Duration(seconds: 1))));

    final expenseQuery = db.selectOnly(db.transactions)
      ..addColumns([db.transactions.amount.sum()])
      ..where(db.transactions.type.equals('EXP') & db.transactions.transactionDate.isBetweenValues(targetMonth, nextMonth.subtract(const Duration(seconds: 1))));
