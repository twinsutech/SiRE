...
Future<DashboardState> dashboardData(DashboardDataRef ref) async {
  ...
  // 4. 최근 12개월 트렌드 (그래프용) - 📍 [수정] 6개월에서 12개월로 확장
  final List<FlSpot> revenueSpots = [];
  final List<FlSpot> expenseSpots = [];
  final now = DateTime.now();

  for (int i = 0; i < 12; i++) {
    // 📍 11개월 전부터 이번 달까지 순차적으로 계산 (11, 10, ... 0)
    final targetMonth = DateTime(now.year, now.month - (11 - i), 1);
    final nextMonth = DateTime(now.year, now.month - (11 - i) + 1, 1);
