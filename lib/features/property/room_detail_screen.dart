    ...
    return StreamBuilder<Unit>(
      ...
      builder: (context, snapshot) {
        ...
        unpaidAsync.whenData((list) {
          final myStatus = list.firstWhere(
                (s) => s.unit.id == currentUnit.id,
            orElse: () => UnpaidStatus(unit: currentUnit, status: 'WAITING', paidAmount: 0, dueDate: DateTime.now()),
          );
          isRealOverdue = myStatus.status == 'OVERDUE';
        });
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text(
              "${currentUnit.roomNumber}${'COMMON_ROOM_UNIT'.tr(ref)} ${'PROP_VIEW_DETAIL'.tr(ref)} $statusText",
              maxLines: 1,
            ...
            actions: [
              IconButton(
                icon: const Icon(Icons.edit),
                tooltip: "COMMON_EDIT".tr(ref),
                onPressed: () => _showEditDialog(context, ref, currentUnit),
              ),
              IconButton(
                icon: const Icon(Icons.delete),
                tooltip: "COMMON_DELETE".tr(ref),
                onPressed: () => _confirmDelete(context, ref, currentUnit),
              ),
            ],
          ),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        currentUnit.memo ?? "PROP_NO_MEMO".tr(ref),
                        style: const TextStyle(fontSize: 16),
                        softWrap: true,
                      ),
                      const SizedBox(height: 20),
      ...
      child: Column(
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: FittedBox(
                        ...
                        child: Text(
                          leaseTypeLabel,
                          style: TextStyle(
                            color: rawLeaseType == '공실' ? Colors.white : AppColors.primaryNavy,
                            fontSize: 12,
                    ...
                    const SizedBox(height: 8),
                    Text(
                      currentUnit.tenantName ?? "PROP_NO_TENANT".tr(ref),
                ...
                Row(
                  children: [
                    CircleAvatar(
                      backgroundColor: Colors.green.withOpacity(0.1),
                      child: IconButton(
                        onPressed: () => _makePhoneCall(currentUnit.tenantPhone ?? ""),
                    ...
                    CircleAvatar(
                      ...
                      child: IconButton(
                        ...
                        tooltip: isOverdue ? "PROP_SEND_REMIND_SMS".tr(ref) : "PROP_SEND_SMS".tr(ref),
                        icon: Icon(
                          isOverdue ? Icons.notification_important : Icons.message,
                          color: isOverdue ? Colors.orange : Colors.blue,
                        ),
          ...
          Row(
            ...
            children: [
              // 📍 [수정] 보증금 및 임대료 금액 표시 다국어 포맷 적용
              Flexible(child: _infoItem("CAT_DEPOSIT".tr(ref), currencyFmt.format(currentUnit.deposit))),
              const SizedBox(width: 12),
              if (rawLeaseType == '월세' || rawLeaseType == '반전세')
                Flexible(child: _infoItem("CAT_RENT".tr(ref), currencyFmt.format(currentUnit.monthlyRent))),
            ],
          ),
        ],
      ),
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  "PROP_CONTRACT_DETAIL_TITLE".tr(ref),
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      "$currentMonthLabel ${'PROP_PAYMENT_STATUS'.tr(ref)}",
                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ),
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          "PROP_MONTHLY_RENT_THIS_MONTH".tr(ref),
                          style: const TextStyle(color: Colors.grey, fontSize: 13),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                        ...
                        FittedBox(
                          ...
                          alignment: Alignment.centerLeft,
                          child: Text(
                            currencyFmt.format(currentUnit.monthlyRent),
                            style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                            maxLines: 1,
                          ),
                        ),
                  ...
                  ElevatedButton.icon(
                    ...
                    label: FittedBox(
                      fit: BoxFit.scaleDown,
                      child: Text(isPaid ? "STATUS_PAID".tr(ref) : "DASHBOARD_PAYMENT_CONFIRM".tr(ref), maxLines: 1),
                    ),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: isPaid ? Colors.grey : AppColors.incomeGreen,
                      foregroundColor: Colors.white,
              ...
              if (isPaid)
                Padding(
                  padding: const EdgeInsets.only(top: 8.0),
                  child: Text(
                    "PROP_ALREADY_RECORDED_MSG".tr(ref),
                    style: const TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.bold),
                  ),
                ),
            ],
    ...
    try {
      ...
      await db.into(db.transactions).insert(
        TransactionsCompanion.insert(
          ...
          transactionDate: now,
          memo: Value(
            "${unit.roomNumber}${l10n.translate('COMMON_ROOM_UNIT')} ${now.month}${l10n.translate('COMMON_MONTH_UNIT')} ${l10n.translate('CAT_RENT')} ${l10n.translate('STATUS_PAID')}",
          ),
        ),
      );
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("${unit.roomNumber}${l10n.translate('COMMON_ROOM_UNIT')} ${l10n.translate('STATUS_PAID')}")),
        );
      }
    } catch (e) {
      if (context.mounted) {
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        image: DecorationImage(image: FileImage(File(img.imagePath)), fit: BoxFit.cover),
                      ),
                    ),
                  ),
                  Positioned(
        ...
        builder: (context, setState) => AlertDialog(
                  ...
                  children: [
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: "PROP_ROOM_NUMBER_LABEL".tr(ref),
                    ...
                    // ✅ [수정] ToggleButtons 오버플로우 방지: 화면 너비를 4등분 + FittedBox로 축소
                    LayoutBuilder(
                    ...
                    if (selectedType != '공실') ...[
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: "PROP_TENANT_NAME_LABEL".tr(ref),
                          border: const OutlineInputBorder(),
                          isDense: true,
                        ),
                      ),
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: "PROP_PHONE_LABEL".tr(ref),
                          border: const OutlineInputBorder(),
                          isDense: true,
                        ),
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: "${'CAT_DEPOSIT'.tr(ref)} ($currencySymbol)",
                          border: const OutlineInputBorder(),
                          isDense: true,
                        ),
                      ...
                      if (selectedType == '월세' || selectedType == '반전세') ...[
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            labelText: "${'CAT_RENT'.tr(ref)} ($currencySymbol)",
                            border: const OutlineInputBorder(),
                            isDense: true,
                          ),
                          keyboardType: TextInputType.number,
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            labelText: "PROP_PAYMENT_DAY_LABEL".tr(ref),
                            border: const OutlineInputBorder(),
                            isDense: true,
                          ),
                          keyboardType: TextInputType.number,
                          ...
                          Expanded(
                            child: OutlinedButton(
                              ...
                              child: FittedBox(
                                fit: BoxFit.scaleDown,
                                child: Text(
                                  startDate == null ? 'PROP_START_DATE'.tr(ref) : DateFormat('yy-MM-dd').format(startDate!),
                                  maxLines: 1,
                                ),
                              ),
                            ),
                          ...
                          Expanded(
                            child: OutlinedButton(
                              ...
                              child: FittedBox(
                                fit: BoxFit.scaleDown,
                                child: Text(
                                  endDate == null ? 'PROP_END_DATE'.tr(ref) : DateFormat('yy-MM-dd').format(endDate!),
                                  maxLines: 1,
                                ),
                              ),
                            ),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: "PROP_ADMIN_MEMO".tr(ref),
                        border: const OutlineInputBorder(),
                        isDense: true,
                      ),
                      maxLines: 2,
          ...
          actions: [
            TextButton(
              ...
              child: Text("COMMON_CANCEL".tr(ref), maxLines: 1, overflow: TextOverflow.ellipsis),
            ),
            ElevatedButton(
              onPressed: () async {
                final db = ref.read(databaseProvider);
              ...
              child: Text("COMMON_SAVE".tr(ref), maxLines: 1, overflow: TextOverflow.ellipsis),
            ),
          ],
        ),
      ),
  ...
  void _confirmDelete(BuildContext context, WidgetRef ref, Unit currentUnit) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("PROP_DELETE_CONFIRM_DESC".tr(ref), maxLines: 3, overflow: TextOverflow.ellipsis),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text("COMMON_CANCEL".tr(ref), maxLines: 1, overflow: TextOverflow.ellipsis),
          ...
          ElevatedButton(
            onPressed: () async {
              final db = ref.read(databaseProvider);
              await (db.delete(db.units)..where((u) => u.id.equals(currentUnit.id))).go();
              ref.invalidate(propertyListProvider);
              ref.invalidate(unpaidListProvider);
              if (context.mounted) {
