...
//                   child: Padding(
//                     padding: const EdgeInsets.symmetric(horizontal: 4),
//                     child: Text(text, style: TextStyle(color: color, fontSize: 10, fontWeight: FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis),
//                   ),
//                 ),
//               ],
//             ),
...
//         ],
//       ),
//       // 📍 [수정] 빌딩이 없을 때 화살표 가이드를 보여주기 위해 Stack으로 감쌈
//       body: Stack(
//         children: [
//           propertyListAsync.when(
//             loading: () => const Center(child: CircularProgressIndicator()),
//             error: (err, stack) => Center(child: Text('Error: $err')),
//             data: (buildingList) {
//               if (buildingList.isEmpty) {
...
//             },
//           ),
//           // 📍 [추가] 빌딩 리스트가 비어있을 때만 나타나는 동적 화살표 가이드
//           propertyListAsync.maybeWhen(
//             data: (list) => list.isEmpty ? const _FloatingGuideArrow() : const SizedBox.shrink(),
...
// }
//
// // 📍 [추가] 리스트가 비어있을 때 + 버튼을 가리키는 동적인 화살표 위젯
// class _FloatingGuideArrow extends StatefulWidget {
//   const _FloatingGuideArrow();
//
//   @override
...
//             offset: Offset(-_animation.value, _animation.value), // 대각선 움직임
//             child: const Column(
//               children: [
//                 Icon(Icons.north_east, size: 40, color: Colors.orangeAccent),
//               ],
//             ),
//           );
//         },
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // 📍 [추가] 빌딩 리스트가 비어있을 때만 나타나는 동적 화살표 가이드 (다국어 라벨 전달)
          propertyListAsync.maybeWhen(
...
// 📍 [추가] 리스트가 비어있을 때 + 버튼을 가리키는 세련된 텍스트 박스 가이드 위젯
class _FloatingGuideArrow extends StatefulWidget {
  final String label; // 📍 다국어 라벨 수신
...
class _FloatingGuideArrowState extends State<_FloatingGuideArrow> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Positioned(
      top: 15, // AppBar 아이콘 높이에 정렬
      right: 55, // + 버튼 왼쪽 옆
      child: AnimatedBuilder(
        animation: _animation,
        builder: (context, child) {
          return Transform.translate(
            offset: Offset(-_animation.value, _animation.value), // 대각선으로 톡톡 튀는 움직임
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              decoration: BoxDecoration(
                color: Colors.orangeAccent.withOpacity(0.9), // 주황색 계열 배경
                borderRadius: BorderRadius.circular(20), // 둥근 박스 처리
                boxShadow: [
                  BoxShadow(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const Icon(
                    Icons.north_east, // + 버튼을 가리키는 화살표 아이콘
                    size: 18,
                    color: Colors.white,
                  ),
                ],
