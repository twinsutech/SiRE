...
// import '../../core/purchase/models/purchase_status.dart';
// import '../../core/purchase/state/purchase_provider.dart'; // ✅ [추가] Pro 상태/구매/복원용
// import '../../core/purchase/ui/refund_policy_sheet.dart'; // 📍 [추가] 환불 정책 바텀시트 연결
// import '../security/security_provider.dart';
// import '../security/pin_screen.dart';
// import 'category_management_screen.dart';
// import 'user_provider.dart';
...
//               subtitle: Text("SETTINGS_SUPPORT_DESC".tr(ref),
//                   maxLines: 1, overflow: TextOverflow.ellipsis),
//               trailing: const Icon(Icons.chevron_right, size: 20), // 📍 아이콘을 통일성 있게 변경
//               onTap: () {
//                 // 📍 [수정] 직접 메일을 열지 않고 환불 정책 바텀시트를 띄웁니다.
...
import '../../core/purchase/ui/refund_policy_sheet.dart';
import '../security/security_provider.dart';
import '../security/pin_screen.dart';
import 'category_management_screen.dart';
import 'user_provider.dart';
import '../../main_screen.dart'; // 📍 [추가] 외부 작업 플래그 제어를 위해 임포트
...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
        ...
        children: [
                  ...
                  if (profile.imagePath != null)
                    Positioned(
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          await ref.read(userNicknameProvider.notifier).updateImagePath(null);
                        },
                        child: Container(
                          decoration: BoxDecoration(
                ...
                onChanged: (value) async {
                  if (value) {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => const PinScreen(isSetting: true)));
                  } else {
                    await _showDeleteConfirmDialog(context, ref);
                  }
                },
              ...
              onTap: (purchaseState.isLoading || isPro)
                ...
                if (latest.errorMessage != null && latest.errorMessage!.isNotEmpty) {
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('IAP_PURCHASE_START_FAILED'.tr(ref) + ": ${latest.errorMessage}"),
                        behavior: SnackBarBehavior.floating,
                      ),
                    );
                  }
          ...
          _buildCard([
            ListTile(
              ...
              onTap: () => RefundPolicySheet.show(context),
            ),
          ]),
          const SizedBox(height: 40),
        ],
  ...
  // ✅ [핵심 수정] 이미지 피커 호출 시 보안 락(Lock) 제어 로직 반영
  Future<void> _pickImage(WidgetRef ref) async {
    // 📍 [추가] 갤러리 진입 전 보안 감시 일시 중지 (무한 루프 방지)
    MainScreen.isExternalActionInProgress = true;
    ...
    // 📍 [추가] 복귀 후 라이프사이클 이벤트가 처리된 뒤 플래그를 해제하기 위해 지연 호출
    Future.delayed(const Duration(milliseconds: 500), () {
      MainScreen.isExternalActionInProgress = false;
    });
  }
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
              suffixIcon: controller.text.isNotEmpty
                  ? GestureDetector(onTap: () { controller.clear(); setState(() {}); }, child: const Icon(Icons.cancel, color: Colors.grey, size: 20))
                  : null,
            ),
          ),
          actions: [
  ...
  Future<void> _handleBackup(BuildContext context, WidgetRef ref) async {
    try {
      ...
      if (await dbFile.exists()) {
        ...
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ERROR_DB_NOT_FOUND".tr(ref))));
      }
    } catch (e) {
      debugPrint("Backup Error: $e");
    }
  ...
  Future<void> _handleRestore(BuildContext context, WidgetRef ref) async {
    ...
    if (result != null && result.files.single.path != null) {
      try {
        ...
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ERROR_RESTORE_FAILED".tr(ref))));
      }
    }
  }
